...
public class FileHandler extends SimpleChannelInboundHandler<HttpObject> {
  ...
  private String downloadPrefix = "/download";
  ...
  private void writeHttpData(InterfaceHttpData data) throws IOException {
    if (data.getHttpDataType() == InterfaceHttpData.HttpDataType.Attribute) {
      ...
      System.out.println("get" + attribute.getName() + " -> " + attribute.getValue());
    } else if (data.getHttpDataType() == InterfaceHttpData.HttpDataType.FileUpload) {
      FileUpload fileUpload = (FileUpload) data;
      if (fileUpload.isCompleted()) {
        String rootDirectory = basePath;
        File dest = new File(rootDirectory, fileUpload.getFilename());
        // 将上传的缓存文件重命名为dest
